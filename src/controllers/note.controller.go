...
import (
	...
	"github.com/naman-paytm/golang-notes-app/src/initializers"
...
func FindNotes(c *fiber.Ctx) error {
	...
	if err != nil {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"status": "fail", "message": "Invalid page number"})
	}

	intLimit, err := strconv.Atoi(limit)
	if err != nil {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"status": "fail", "message": "Invalid limit"})
	}
